...
public class ActivityPlanService {
	...
	public void createActivityPlan(ActivityPlanRequest activityPlanRequest) {
		Planlist planlist = planlistRepository.findById(activityPlanRequest.getPlanId()).orElse(null); // planlist의 PlanId를 불러와 저장
		Tourinfo tourinfo = tourinfoRepository.findById(activityPlanRequest.getContentId()).orElse(null); // tourinfo의 ContentId를 불러와 저장 
		ActivityPlan activityPlan = ActivityPlan.builder()
	            .contentS(activityPlanRequest.getContentS()) // 입력받은 ContentS를 activityPlan에 저장
	            .contentE(activityPlanRequest.getContentE()) // 입력받은 ContentE를 activityPlan에 저장
	            .dayIndex(activityPlanRequest.getDayIndex())
	            .planlist(planlist)
	            .tourinfo(tourinfo)
	            .build();
		
	    System.out.println("PlanId: " + activityPlanRequest.getPlanId());
	    System.out.println("ContentId: " + activityPlanRequest.getContentId());
	    activityPlanRepository.save(activityPlan);
	}

	public List<ActivityPlan> findByPlanId(Long planId) {
		List<ActivityPlan> list = activityPlanRepository.findByPlanlistPlanId(planId);
		return list;
		
	}

	public ActivityPlan updateActivityPlan(Long id, ActivityPlan updatedActivityPlan) {
		return activityPlanRepository.findById(id).map(updatePlan -> {
			updatePlan.setContentS(updatedActivityPlan.getContentS()); // 여행시작시간 수정 
			updatePlan.setContentE(updatedActivityPlan.getContentE()); // 여행종료시간 수정
			updatePlan.setDayIndex(updatedActivityPlan.getDayIndex()); // N일차 수정
	...
	public void deleteActivityPlan(Long id) {
		activityPlanRepository.deleteById(id); // 일정을 삭제
	}
