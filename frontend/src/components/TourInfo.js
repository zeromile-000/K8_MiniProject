...
const TourInfo = () => {
  ...
  const loadTourInfos = async (page) => {
    setIsLoading(true);
    try {
      const url = `http://localhost:8080/tourinfo?pageNo=1&addr1=2&addr2=1&category=12`;
...
// const TourInfo = () => {
//   const [tourInfos, setTourInfos] = useState([]); // 전체 Tour 정보
//   const [filteredTourInfos, setFilteredTourInfos] = useState([]); // 필터링된 Tour 정보
//   const [pageNo, setPageNo] = useState(1); // 현재 페이지 번호
//   const [totalPages, setTotalPages] = useState(1); // 전체 페이지 수
//   const [isLoading, setIsLoading] = useState(false); // 로딩 상태
...
//   const [selectedSigunguCode, setSelectedSigunguCode] = useState("");
//   const [selectedContentTypeId, setSelectedContentTypeId] = useState("");
//   const [filteredSigungu, setFilteredSigungu] = useState([]); // 시/군/구 필터링
//   const navigate = useNavigate(); // 페이지 이동을 위한 navigate
...
//     setIsLoading(true);
//     try {
//       const response = await axios.get(`http://localhost:8080/tourinfo?pageNo={1}`);
//       const newTourInfos = response.data || [];
//       setTourInfos(newTourInfos);
//       setFilteredTourInfos(newTourInfos); // 초기 데이터 설정
//       setTotalPages(Math.ceil(newTourInfos.length / 10)); // 페이지 계산 (한 페이지당 10개 기준)
//     } catch (error) {
//       console.error("Error fetching tour info:", error);
//     } finally {
//       setIsLoading(false);
...
//   useEffect(() => {
//     const fetchFilteredTourInfos = async () => {
//       if (!selectedAreaCode || !selectedSigunguCode || !selectedContentTypeId) {
//         setFilteredTourInfos(tourInfos);
//         return;
//       }
...
//         setFilteredTourInfos(filteredData);
//         setTotalPages(Math.ceil(filteredData.length / 10));
//         setPageNo(1); // 필터 변경 시 첫 페이지로 초기화
//       } catch (error) {
//         console.error("Error fetching filtered tour info:", error);
//       } finally {
//         setIsLoading(false);
...
//     fetchFilteredTourInfos();
//   }, [selectedAreaCode, selectedSigunguCode, selectedContentTypeId, tourInfos]);
...
//     }
//   };
//   console.log("filteredTourInfos", filteredTourInfos)
//   // 현재 페이지의 데이터 가져오기
//   const currentPageData = filteredTourInfos.slice((pageNo - 1) * 10, pageNo * 10);
